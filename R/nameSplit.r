...
nameSplit <- function(splist){
  ...
  nameSplit_ck <- function(sp){
    ...
    sp <- gsub(paste(paste(" ", c("nothovar.","nothovar")," ",sep=""), collapse="|"), " nothovar. ", sp, ignore.case=TRUE)
    sp <- gsub(paste(paste(" ", c("nothof.","nothof")," ",sep=""), collapse="|"), " nothof. ", sp, ignore.case=TRUE)
