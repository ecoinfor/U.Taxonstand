...
#'
#' @param isPlant Logical. If TRUE(default), the function tried to use one datafile from R package Taxonstand for fuzzy matching of genus names.
#'
...
#'
#'@references Zhang, J. & Qian, H. U.Taxonstand: An R package for standardizing scientific names of plants and animals. In prep.
#'
#'@examples
#'## load the database
...
#'
#'@export
nameMatch <- function(spList=NULL, spSource=NULL, author = TRUE, max.distance= 1, isPlant=TRUE, genusPairs=NULL)
{
  ################################################
  ################  The main  function for name matching

  ## Install the required packages
  list.of.packages <- c("plyr","magrittr")
  ...
  ##------------
  print("Loading and processing the backbone database...")
  ...
  if(!"GENUS"%in%colnames(spSource)){
    ...
    spSource$GENUS <- suppressWarnings(splitName_spSource%>%sapply(extract2, 1))
  }
  ...
  if(!"GENUS"%in%colnames(spList)){
    ...
    spList$GENUS <- suppressWarnings(strsplit(spList$NAMECLEAN, " ")%>%sapply(extract2, 1))
  }
  ...
  if(nrow(spList02)>0){
    ...
    if(isPlant==TRUE & length(genusRepl)>0){
      #--- To match with the data file "lista", which is from the R package Taxonstand, and "lista_2" which is added by us.
      row.fn <- function(columna, error, key, lista){
        for (i in 1:length(lista[, columna])) {
          dat = adist(key, trimws(lista[i, columna])) <=
            error
          if ((lista[i, columna]) == "")
            break
          if (dat == TRUE)
      ...
      for(i in 1:length(genusRepl)){
        ...
        resultado <- trimws(row.fn(letra1, 0, genero, lista))

        if (resultado[1] == "" && sum(adist(genero, trimws(lista[,
                                                                 letra1])) <= 1) > 1 || sum(adist(genero, trimws(lista[,
                                                                                                                       letra2])) <= 1) > 1) {
        ...
        rm(resultado, letra1, genero,letra2)
      }
